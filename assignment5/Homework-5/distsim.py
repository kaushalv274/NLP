...
def cossim_sparse(v1,v2):
    ...
    for key, value in v2.items():
        mod2 += (value ** 2)
    return cos / math.sqrt(mod1 * mod2)
...
def cossim_dense(v1,v2):
    ...
    return np.sum(v1*v2)/math.sqrt(np.sum(np.square(v1))*np.sum(np.square(v2)))
    ## TODO: delete this line and implement me
...
def show_nearest(word_2_vec, w_vec, exclude_w, sim_metric):
    ...
    for key, value in word_2_vec.items():
        if key not in exclude_w:
            val = sim_metric(value, w_vec)
            answer.append((key, val))
    ans = sorted(answer, key = lambda x : x[1], reverse=True)
    return ans[:10]
