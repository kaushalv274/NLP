...
class Tree(object):
    ...
    def head_annotate (self):
        nodes = list(self.bottomup())
        for node in nodes:
            if (len(node.children) >= 2) and (node.parent is not None) and ('NP' in node.label or 'VP' in node.label):
                node.label = node.label + '^' + node.parent.label

    def rev_head_annotate(self):
        nodes = list(self.bottomup())
        for node in nodes:
            node.label = node.label.split('^')[0]

    def head_annotate_gp(self):
        nodes = list(self.bottomup())
        for node in nodes:
            if (len(node.children) >= 2) and (node.parent is not None) and ('NP' in node.label or 'VP' in node.label):
                if node.parent.parent is not None:
                    node.label  = node.label + '^' + node.parent.label + '^' + node.parent.parent.label
                else:
                    node.label = node.label + '^' + node.parent.label

    def rev_head_annotate_gp(self):
        nodes = list(self.bottomup())
        for node in nodes:
            node.label = node.label.split('^')[0]

    def pre_terminal (self):
        nodes = list(self.bottomup())
        for node in nodes:
            if (len(node.children) == 1) and (node.parent is not None):
                node.label = node.label + '^' + node.parent.label

    def pre_terminal_rev(self):
        nodes = list(self.bottomup())
        for node in nodes:
            node.label = node.label.split('^')[0]
